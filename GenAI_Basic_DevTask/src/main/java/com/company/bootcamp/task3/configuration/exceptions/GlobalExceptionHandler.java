...
public class GlobalExceptionHandler implements WebExceptionHandler {
    ...
    public Mono<Void> handle(ServerWebExchange exchange, Throwable ex) {
        if (ex instanceof IOException) {
            return handleIOException((IOException) ex, exchange);
        } else if (ex instanceof RestClientException) {
            return handleRestClientException((RestClientException) ex, exchange);
        } else if (ex instanceof ModelNotFoundException) {
            return handleModelNotFoundException((ModelNotFoundException) ex, exchange);
        } else {
            return handleGeneralException(ex, exchange);
        }
    }
